...
== 總結
以下是XMLHttpRequest與fetch的比較

[options="header", cols="3*"]
|===
| 比較項目 | Fetch API | XMLHttpRequest

| **語法**
| 基於 Promise，語法更現代化、簡潔。
| 基於事件的回調函數，語法較為冗長。

| **基於 Promises**
| 是，使用 `then` 和 `catch` 處理異步操作。
| 否，依賴事件和回調函數處理異步操作。
...
| **錯誤處理**
| 更加直觀，使用 `try...catch` 可以捕捉異常。
| 錯誤處理不夠直觀，需要依賴事件監聽。

| **上傳/下載進度**
| 原生不支持，需要使用 `ReadableStream` 等進階技術。
| 支持 `onprogress` 事件來監控進度。

| **CORS 支持**
| 原生支持跨域請求，且默認不攜帶憑證。
| 支持跨域，但需要設置 `withCredentials` 屬性。

| **返回格式**
| 支持多種格式，如 `json()`, `blob()`, `text()` 等。
| 主要通過 `responseText` 或 `responseXML` 獲取。
...
| **流式處理**
| 支持流式讀取響應數據，適合處理大數據或實時數據。
| 不支持流式處理，需等待整個響應完成後再處理。
|===
